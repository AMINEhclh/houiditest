  ...
  const renderStep = () => {
    switch (step) {
          ...
          >
            ...
            <div className="flex flex-col items-center gap-4 pt-4">
              <p className="text-sm md:text-base font-bold text-muted-foreground">
                Answer 4 quick questions to unlock your custom growth roadmap.
              </p>
              <Button 
                onClick={nextStep} 
      ...
      case 5:
        ...
        const field = step === 3 ? "monthlyClients" : step === 4 ? "monthlyRevenue" : "businessType";
        ...
        return (
          ...
          >
            ...
            {step === 5 && data.businessType === "Other" && (
              <div className="max-w-lg mx-auto space-y-3">
                <Input 
                  placeholder="Specify business type" 
                  value={data.otherBusinessType}
                  onChange={(e) => handleDataChange("otherBusinessType", e.target.value)}
                  className="h-14 rounded-xl bg-white/5 border-white/10 text-white"
                />
                <Button onClick={nextStep} disabled={!data.otherBusinessType} className="w-full h-14 rounded-xl bg-[#1DB954]">Continue</Button>
              </div>
            )}
          </motion.div>
        );
